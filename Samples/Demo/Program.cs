...
namespace LinuxRfidSample
{
    internal class Program
    {
        private delegate void CommandFunc(Reader rdr, ArgParser pargs);
        ...
        private static void Main(string[] args)
        {
            try
            {
                ...
                if (pargs.keywordArgs.ContainsKey("log"))
                {
                    ...
                    //// ORIGINAL CODE: tl = delegate (object sender, TransportListenerEventArgs e) { ... }
                    rdr.Transport += (object? sender, TransportListenerEventArgs e) =>
                    {
                        string format = "eapi";
                        ...
                        logwriter.WriteLine(String.Format("{0}: {1} (timeout={2:D}ms)", e.Tx ? "TX" : "RX", msg, e.Timeout));
                        logwriter.Flush();
                    };
                }

                //// ORIGINAL CODE: rdr.Transport += tl;
                ...
                try
                {
                        ...
                        ("script" == pargs.command.ToLower()))
                    {
                        ...
                        {
                            if (pargs.commandArgs.Length < 1)
                            {
                                throw new UsageException("script command must be followed by a filename");
                            }
                        ...
                        NextLine:
                            string line = (inStream != null) ? inStream.ReadLine() : Console.ReadLine();
                            if (null == line) break;
        ...
        private static void AddCommand(string name, CommandFunc handler)
        {
        ...
        private static void RunCommand(Reader rdr, ArgParser pargs)
        {
            try
            {
                try
        ...
        private enum ReadLevel3Type
        { BYCOUNT, BYCOUNT496, BYINDEX, BYINDEX496, WITHMETADATA };
        ...
        private delegate void WritePasswordFunc(Reader rdr, TagFilter target, Gen2.Password password);
    ...
    "Run internal Gen2 constructor tests");
        ...
        private static void PrintTagReadHandler(Object sender, TagReadDataEventArgs e)
        {
            PrintTagRead(e.TagReadData);
        }

        private static void PrintTagRead(TagReadData read)
        {
        ...
        private static void PrintTagReads(TagReadData[] reads)
        {
            foreach (TagReadData read in reads)
            {
                PrintTagRead(read);
        ...
        private static void PrintTagDatas(TagData[] reads)
        {
        ...
        private static void ReadExceptionHandler(Object sender, ReaderExceptionEventArgs e)
        {
